...
> **Fonte de verdade: a página [`/brand`](app/brand/page.tsx)** (`strutura.ai/brand`, seção 11; componentes em `components/brand/ds/`): a página renderiza tokens, tipografia, grão e exemplos ao vivo, além da identidade estratégica da marca. O antigo `design.html` foi aposentado (redirect permanente `/design.html` → `/brand`). Este arquivo é o resumo de trabalho para consulta rápida. **Consultar antes de criar ou alterar qualquer peça.**
...
| Função | Carrega o grosso do conteúdo, inclusive o denso. Acolhe e lê como ambiente arrumado (o público é o empreendedor no digital, não-técnico) | Contraste e quebra de ritmo: o "susto visual". Retenção de perfil, não decoração |
...
- Escuro como default (afasta o não-técnico, o empreendedor no digital)
...
_Última atualização: 22 de julho de 2026. Design system "Nature Warm" (v3), substituindo o "Warm Minimal" (v2); site migrado para os tokens Nature Warm. Reposicionamento jul/2026 refletido: público = empreendedor no digital (rótulo atualizado junto com a bio do Instagram), racional do Campo re-ancorado na missão (controle através de organização). Fonte de verdade: a página [`/brand`](app/brand/page.tsx) (`strutura.ai/brand`, seção 11)._
